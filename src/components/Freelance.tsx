...
export default function Freelance() {
  ...
  return (
    <section id="freelance" ref={ref} className="py-16 sm:py-20 bg-neutral-200">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {projects.map((project) => (
            <div key={project.id} className="card overflow-hidden hover:border-green-600 transition-colors">
              {/* Project Image */}
              ...
              <div className="p-5">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.technologies.slice(0, 4).map((tech) => (
                    <span key={tech} className="px-2 py-1 bg-neutral-300 text-neutral-800 text-xs rounded-full">
                      {tech}
                    </span>
                  ))}
                  {project.technologies.length > 4 && (
                    <span className="px-2 py-1 bg-neutral-300 text-neutral-500 text-xs rounded-full">
                      +{project.technologies.length - 4}
                    </span>
                  )}
                </div>
                ...
                <div className="flex gap-2">
                  {project.liveUrl !== "#" && (
                    <a
                      ...
                      className="flex-1 px-3 py-3 sm:py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-1 touch-target"
                    >
                  ...
                  {project.githubUrl !== "#" && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-1 px-3 py-3 sm:py-2 bg-neutral-300 hover:bg-neutral-400 text-neutral-800 text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-1 touch-target"
                    >
                      <Github size={14} />
                      Code
                    </a>
        ...
        <div className="text-center mt-16">
          ...
          <a
            href="#contact"
            className="inline-flex items-center gap-2 px-8 py-4 bg-green-600 hover:bg-green-700 text-white rounded-lg font-semibold transition-colors"
          >
            Let&apos;s Talk
          </a>
        </div>
      ...
      {isModalOpen && selectedProject && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <button
            onClick={closeModal}
            className="absolute top-4 right-4 z-[60] p-3 bg-white hover:bg-neutral-200 rounded-full shadow-lg touch-target"
                ...
                {selectedProject.images.length > 1 && (
                  <>
                    <button
                      ...
                      className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 p-2 sm:p-3 bg-white/90 hover:bg-white rounded-full shadow-lg touch-target z-10"
                    ...
                    <button
                      ...
                      className="absolute right-2 sm:right-4 top-1/2 -translate-y-1/2 p-2 sm:p-3 bg-white/90 hover:bg-white rounded-full shadow-lg touch-target z-10"
                  ...
                  <div className="absolute bottom-2 left-2 right-2 flex gap-2 overflow-x-auto pb-2">
                    {selectedProject.images.map((image, index) => (
                      <button
                        ...
                        className={`relative w-14 h-14 sm:w-16 sm:h-16 flex-shrink-0 rounded-lg overflow-hidden border-2 touch-target ${
